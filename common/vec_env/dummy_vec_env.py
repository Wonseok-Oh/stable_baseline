...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        ...
        env = self.envs[0]
        VecEnv.__init__(self, len(env_fns), env.observation_space, env.action_space, env.meta_action_space)
    ...
    def invoke(self, meta_actions):
        """
        Step the environments with the given action

        :param meta_actions: ([int] or [float]) the action
        :return: ([int] or [float], [float], [bool], dict) observation, reward, done, information
        """
        self.invoke_async(meta_actions)
        return self.invoke_wait()
    
    def invoke_async(self, meta_actions):
    ...
    def invoke_wait(self):
        for env_idx in range(self.num_envs):
            ...
            #print("self.meta_actions[env_idx]: {}".format(self.meta_actions[env_idx]))
            obs, self.buf_rews[env_idx], self.buf_dones[env_idx], self.buf_infos[env_idx] =\
                self.envs[env_idx].invoke(self.meta_actions[env_idx])
            if self.buf_dones[env_idx]:
                # save final observation where user can get it, then reset
                self.buf_infos[env_idx]['terminal_observation'] = obs
                obs = self.envs[env_idx].reset()
