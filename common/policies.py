...
class BasePolicy(ABC):
    ...
    def __init__(self, sess, ob_space, meta_ac_space, n_env, n_steps, n_batch, reuse=False, scale=False,
                 obs_phs=None, add_meta_action_ph=False):
        self.n_env = n_env
        ...
        with tf.variable_scope("input", reuse=False):
            ...
            self._meta_action_ph = None
            if add_meta_action_ph:
                self._meta_action_ph = tf.placeholder(dtype=ac_space.dtype, shape=(n_batch,) + meta_ac_space.shape,
                                                 name="meta_action_ph")
        self.sess = sess
        self.reuse = reuse
        self.ob_space = ob_space
        self.meta_ac_space = meta_ac_space
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            self._meta_action = self.proba_distribution.sample()
            self._deterministic_meta_action = self.proba_distribution.mode()
            self._neglogp = self.proba_distribution.neglogp(self.meta_action)
    ...
    @property
    def deterministic_meta_action(self):
        """tf.Tensor: deterministic action, of shape (self.n_batch, ) + self.ac_space.shape."""
        return self._deterministic_meta_action
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, meta_ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
                 act_fun=tf.tanh, cnn_extractor=nature_cnn, feature_extraction="cnn", **kwargs):
        super(FeedForwardPolicy, self).__init__(sess, ob_space, meta_ac_space, n_env, n_steps, n_batch, reuse=reuse,
                                                scale=(feature_extraction == "cnn"))
    ...
    def invoke(self, obs, state=None, mask=None, deterministic=False):
        if deterministic:
            meta_action, value, neglogp = self.sess.run([self.deterministic_meta_action, self.value_flat, self.neglogp],
                                                   {self.obs_ph: obs})
        else:
            meta_action, value, neglogp = self.sess.run([self.meta_action, self.value_flat, self.neglogp],
                                                   {self.obs_ph: obs})
        return meta_action, value, self.initial_state, neglogp
        
    def proba_invoke(self, obs, state=None, mask=None):
