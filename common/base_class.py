...
class BaseRLModel(ABC):
    ...
    def __init__(self, policy, env, verbose=0, *, requires_vec_env, policy_base,
        ...
        self.meta_action_space = None
        self.n_envs = None
        self._vectorize_action = False
        self._vectorize_meta_action = False
        ...
        if env is not None:
            ...
            self.meta_action_space = env.meta_action_space
            if requires_vec_env:
                ...
                if isinstance(env, VecEnv):
                    if env.num_envs == 1:
                        ...
                        self._vectorize_meta_action = True
                    else:
    ...
    def set_env(self, env):
        ...
        assert self.meta_action_space == env.meta_action_space, \
            "Error: the environment passed must have at least the same meta_action space as the model was trained on."
    ...
    def set_random_seed(self, seed: Optional[int]) -> None:
        ...
        if self.env is not None:
            self.env.seed(seed)
            # Seed the meta_action space
            # useful when selecting random meta_actions
            self.env.meta_action_space.seed(seed)
        self.meta_action_space.seed(seed)
    ...
    def pretrain(self, dataset, n_epochs=10, learning_rate=1e-4,
        ...
        """
        continuous_meta_actions = isinstance(self.meta_action_space, gym.spaces.Box)
        discrete_meta_actions = isinstance(self.meta_action_space, gym.spaces.Discrete)

        assert discrete_meta_actions or continuous_meta_actions, 'Only Discrete and Box meta_action spaces are supported'
        ...
        with self.graph.as_default():
            with tf.variable_scope('pretrain'):
                if continuous_meta_actions:
                    obs_ph, meta_actions_ph, deterministic_meta_actions_ph = self._get_pretrain_placeholders()
                    loss = tf.reduce_mean(tf.square(meta_actions_ph - deterministic_meta_actions_ph))
                else:
                    obs_ph, meta_actions_ph, meta_actions_logits_ph = self._get_pretrain_placeholders()
                    # meta_actions_ph has a shape if (n_batch,), we reshape it to (n_batch, 1)
                    # so no additional changes is needed in the dataloader
                    meta_actions_ph = tf.expand_dims(meta_actions_ph, axis=1)
                    one_hot_meta_actions = tf.one_hot(meta_actions_ph, self.meta_action_space.n)
                    loss = tf.nn.softmax_cross_entropy_with_logits_v2(
                        logits=meta_actions_logits_ph,
                        labels=tf.stop_gradient(one_hot_meta_actions)
                    )
        ...
        for epoch_idx in range(int(n_epochs)):
            ...
            for _ in range(len(dataset.train_loader)):
                expert_obs, expert_meta_actions = dataset.get_next_batch('train')
                feed_dict = {
                    obs_ph: expert_obs,
                    meta_actions_ph: expert_meta_actions,
                }
            ...
            if self.verbose > 0 and (epoch_idx + 1) % val_interval == 0:
                ...
                for _ in range(len(dataset.val_loader)):
                    expert_obs, expert_meta_actions = dataset.get_next_batch('val')
                ...
                if self.verbose > 0:
                    ...
                    print()
            # Free memory
            del expert_obs, expert_meta_actions
        if self.verbose > 0:
    ...
    def predict(self, observation, state=None, mask=None, deterministic=False):
        """
        Get the model's meta_action from an observation

        :param observation: (np.ndarray) the input observation
        :param state: (np.ndarray) The last states (can be None, used in recurrent policies)
        :param mask: (np.ndarray) The last masks (can be None, used in recurrent policies)
        :param deterministic: (bool) Whether or not to return deterministic meta_actions.
        :return: (np.ndarray, np.ndarray) the model's meta_action and the next state (used in recurrent policies)
        """
        pass

    @abstractmethod
    def meta_action_probability(self, observation, state=None, mask=None, meta_actions=None, logp=False):
        """
        If ``meta_actions`` is ``None``, then get the model's meta_action probability distribution from a given observation.

        Depending on the meta_action space the output is:
            - Discrete: probability for each possible meta_action
            - Box: mean and standard deviation of the meta_action output

        However if ``meta_actions`` is not ``None``, this function will return the probability that the given meta_actions are
        taken with the given parameters (observation, state, ...) on this model. For discrete meta_action spaces, it
        returns the probability mass; for continuous meta_action spaces, the probability density. This is since the
        ...
        :param state: (np.ndarray) The last states (can be None, used in recurrent policies)
        :param mask: (np.ndarray) The last masks (can be None, used in recurrent policies)
        :param meta_actions: (np.ndarray) (OPTIONAL) For calculating the likelihood that the given meta_actions are chosen by
            the model for each of the given parameters. Must have the same number of meta_actions and observations.
            (set to None to return the complete meta_action probability distribution)
        :param logp: (bool) (OPTIONAL) When specified with meta_actions, returns probability in log-space.
            This has no effect if meta_actions is None.
        :return: (np.ndarray) the model's (log) meta_action probability
        """
        pass
...
class ActorCriticRLModel(BaseRLModel):
    ...
    def predict(self, observation, state=None, mask=None, deterministic=False):
        ...
        meta_actions, _, states, _ = self.invoke(observation, state, mask, deterministic=deterministic)

        clipped_meta_actions = meta_actions
        # Clip the meta_actions to avoid out of bound error
        if isinstance(self.meta_action_space, gym.spaces.Box):
            clipped_meta_actions = np.clip(meta_actions, self.meta_action_space.low, self.meta_action_space.high)

        if not vectorized_env:
            if state is not None:
                raise ValueError("Error: The environment must be vectorized when using recurrent policies.")
            clipped_meta_actions = clipped_meta_actions[0]

        return clipped_meta_actions, states
    ...
    def meta_action_probability(self, observation, state=None, mask=None, meta_actions=None, logp=False):
        ...
        observation = observation.reshape((-1,) + self.observation_space.shape)
        meta_actions_proba = self.proba_invoke(observation, state, mask)

        if len(meta_actions_proba) == 0:  # empty list means not implemented
            warnings.warn("Warning: meta_action probability is not implemented for {} meta_action space. Returning None."
                          .format(type(self.meta_action_space).__name__))
            return None

        if meta_actions is not None:  # comparing the meta_action distribution, to given meta_actions
            prob = None
            logprob = None
            meta_actions = np.array([meta_actions])
            if isinstance(self.meta_action_space, gym.spaces.Discrete):
                meta_actions = meta_actions.reshape((-1,))
                assert observation.shape[0] == meta_actions.shape[0], \
                    "Error: batch sizes differ for meta_actions and observations."
                prob = meta_actions_proba[np.arange(meta_actions.shape[0]), meta_actions]

            elif isinstance(self.meta_action_space, gym.spaces.MultiDiscrete):
                meta_actions = meta_actions.reshape((-1, len(self.meta_action_space.nvec)))
                assert observation.shape[0] == meta_actions.shape[0], \
                    "Error: batch sizes differ for actions and observations."
                # Discrete action probability, over multiple categories
                meta_actions = np.swapaxes(meta_actions, 0, 1)  # swap axis for easier categorical split
                prob = np.prod([proba[np.arange(act.shape[0]), act]
                                         for proba, act in zip(meta_actions_proba, meta_actions)], axis=0)

            elif isinstance(self.meta_action_space, gym.spaces.MultiBinary):
                meta_actions = meta_actions.reshape((-1, self.meta_action_space.n))
                assert observation.shape[0] == meta_actions.shape[0], \
                    "Error: batch sizes differ for meta_actions and observations."
                # Bernoulli meta_action probability, for every meta_action
                prob = np.prod(meta_actions_proba * meta_actions + (1 - meta_actions_proba) * (1 - meta_actions), axis=1)

            elif isinstance(self.meta_action_space, gym.spaces.Box):
                meta_actions = meta_actions.reshape((-1, ) + self.meta_action_space.shape)
                mean, std = meta_actions_proba
                logstd = np.log(std)

                n_elts = np.prod(mean.shape[1:])  # first dimension is batch size
                log_normalizer = n_elts / 2.0 * np.log(2 * np.pi) + np.sum(logstd, axis=1)

                # Diagonal Gaussian meta_action probability, for every meta_action
                logprob = -np.sum(np.square(meta_actions - mean) / (2 * np.square(std)), axis=1) - log_normalizer

            else:
                warnings.warn("Warning: meta_action_probability not implemented for {} meta_actions space. Returning None."
                              .format(type(self.meta_action_space).__name__))
                return None
            ...
            # normalize meta_action proba shape for the different gym spaces
            ret = ret.reshape((-1, 1))
        else:
            ret = meta_actions_proba
